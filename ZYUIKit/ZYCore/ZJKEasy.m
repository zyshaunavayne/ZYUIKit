...
//  ZJKKit
//
//  Created by Fan Li Lin on 2022/3/17.
//  Copyright © 2022 zhong jun ke ji. All rights reserved.
//

#import "ZJKEasy.h"
#import "ZJKRouterNP.h"

CGFloat ZJKScreenWidth(void)
{
    return [UIScreen mainScreen].bounds.size.width;
}

CGFloat ZJKScreenHeight(void)
{
    return [UIScreen mainScreen].bounds.size.height;
}

UIWindow *ZJKMainWindow(void)
{
    UIWindow *window = nil;
...
BOOL ZJKIsIphoneX(void)
{
    BOOL isPhoneX = NO;
    if (@available(iOS 11.0, *)) {
        isPhoneX = ZJKMainWindow().safeAreaInsets.bottom > 0.0;
    }
    return isPhoneX;
}

CGFloat ZJKSafeAreaBottomHeight(void)
{
    CGFloat safeHeight = CGFLOAT_MIN;
    if (@available(iOS 11.0, *)) {
        safeHeight = ZJKMainWindow().safeAreaInsets.bottom;
    }
    return safeHeight;
}

CGFloat ZJKStatusBarHeight(void)
{
...
void ZJKSetDeviceOrientation(UIDeviceOrientation orientation)
{
    @try {
        // ios16使用新的api
        if (@available(iOS 16.0, *)) {
            ...
            UINavigationController *nav = ZJKRouterNP.currentViewController.navigationController;
